...
def download_5tokens_trained_model():
    ...
    if not pretrained_model.exists():
        ...
        gdown.download(id='19s68NxHybYta0t-smhWKrGZxUYyRlgHA&confirm=t', output=dst_file, quiet=False)
...
def download_4tokens_trained_model():
    ...
    if not pretrained_model.exists():
        ...
        gdown.download(id='19sDdfrnRDAeL6bKraKxb8mrI9t7JCdVE&confirm=t', output=dst_file, quiet=False)
        if Path(dst_file).exists():
            gdown.extractall(dst_file, str(EXP_DIR))
            os.remove(dst_file)