...
from source.resources import DUMPS_DIR, PHASES, WORD_FREQUENCY_FILEPATH, \
    get_data_filepath, PROCESSED_DATA_DIR, \
    DATASETS_DIR, WIKILARGE_DATASET, WORD_EMBEDDINGS_NAME, download_glove, NEWSELA_GRADE_DATASET, AOA_PATH
...
@lru_cache(maxsize=1024)
def get_AoA_feats(sentence):
    words = tokenize(remove_stopwords(remove_punctuation(sentence)))
    vals = []
    for word in words:
        if word in aoa_df["Word"].tolist():
            vals.append(aoa_df[aoa_df.Word == word]["Rating.Mean"].to_list()[0])
    return np.mean(vals), np.std(vals)


@lru_cache(maxsize=1024)
def get_syllables_feats(sentence):
    words = tokenize(remove_stopwords(remove_punctuation(sentence)))
    vals = []
    for word in words:
        vals.append(syllables.estimate(word))
    return np.mean(vals), np.std(vals)
...
# Changed Distance -> Similarity
class ReplaceOnlyLevenshteinRatioFeature(RatioFeature):
    def __init__(self, *args, **kwargs):
        super().__init__(self.get_replace_only_levenshtein_similarity, *args, **kwargs)

    def get_replace_only_levenshtein_distance_ratio(self, complex_sentence, simple_sentence):
        max_replace_only_distance = min(len(complex_sentence), len(simple_sentence))
        return safe_division(
            self.get_replace_only_levenshtein_distance(complex_sentence, simple_sentence), max_replace_only_distance
        )

    def get_replace_only_levenshtein_similarity(self, complex_sentence, simple_sentence):
        return round(1 - self.get_replace_only_levenshtein_distance_ratio(complex_sentence, simple_sentence))

    def get_replace_only_levenshtein_distance(self, complex_sentence, simple_sentence):
        return len(
...
if __name__ == '__main__':
    ...
    # preprocessor.preprocess_dataset(NEWSELA_GRADE_DATASET)
